...
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³ gEspI003 ³ Autor ³ George AC. Gonçalves ³ Data ³ 18/06/09  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Funções   ³ gEspI003 ³ Autor ³ George AC. Gonçalves ³ Data ³ 18/06/09  ³±±
±±³          ³ gVTrfDep ³ Autor ³ George AC. Gonçalves ³ Data ³ 18/06/09  ³±±
±±³          ³ gWFHelpD ³ Autor ³ George AC. Gonçalves ³ Data ³ 18/06/09  ³±±
±±³          ³ gWFGestor³ Autor ³ George AC. Gonçalves ³ Data ³ 18/06/09  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descrição ³ Manutenção na Transferência Departamental                  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ Específio: Projeto de concessão de acesso                  ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±³Partida   ³ Seleção da opção transf. departamental - Rotina gEspM005   ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
User Function gEspI003()  // Manutenção na Transferência Departamental
...
User Function gVTrfDep()  // validação digitação da transferência
...
If Empty(M->ZZE_CDUSU)  // se códifo de usuário não informado
	MsgStop("Não é possivel gravar solicitação sem o preenchimento do código de usuário","Atenção")
	lRet := .F.  // retorno da função					
EndIf	

DbSelectArea("ZZD")  // seleciona arquivo de módulo/perfil conflitante
ZZD->(DbSetOrder(1))  // muda ordem do índice
ZZD->(DbGoTop())  // vai para o início do arquivo

Do While ZZD->(!Eof())  // percorre todo o arquivo
	For gLn1 := 1 To Len(aCols)  // percorre todos os itens da solicitação
		If aCols[gLn1,Len(aCols[gLn1])] == .F.  // se linha não deletada							
			...
			If ZZC->(DbSeek(xFilial("ZZC")+gcModO+gcPerfO))  // posiciona registro		
				...
				If ZZD->ZZD_CDMODO+ZZD->ZZD_CDPERO == gcModO+gcPerfO  // se existe o mófulo/perfil origem
					For gLn2 := 1 To Len(aCols)  // percorre todos os itens da solicitação
						If aCols[gLn2,Len(aCols[gLn2])] == .F.  // se linha não deletada											
							...
							If ZZC->(DbSeek(xFilial("ZZC")+gcModC+gcPerfC))  // posiciona registro
								...
								If ZZD->ZZD_CDMODC+ZZD->ZZD_CDPERC == gcModC+gcPerfC  // se existe o mófulo/perfil conflitante
									lRet := .F.  // retorno da função				
									MsgStop("O módulo/perfil ["+gcModO+"/"+gcPerfO+" ("+cDescO+")] é conflitante com o módulo/perfil ["+gcModC+"/"+gcPerfC+" ("+cDescC+")]. Não é possível gravar a solicitação com perfis conflitantes.","Atenção")
								EndIf
							EndIf	
						EndIf
					Next gLn2
...
cSubject := "Abertura da solicitação de alteração de perfil de acesso para o usuário: "+M->ZZE_NMUSU  // Assunto da mensagem
