...
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³ gEspI001 ³ Autor ³ George AC. Gonçalves ³ Data ³ 19/12/08  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Funções   ³ gEspI001 ³ Autor ³ George AC. Gonçalves ³ Data ³ 26/05/09  ³±±
±±³          ³ gVSolic  ³ Autor ³ George AC. Gonçalves ³ Data ³ 27/12/08  ³±±
...
User Function gEspI001()  // Manutenção na Solicitação de Perfil de Acesso
...
///	cTudOk         := "AllwaysTrue()"
	...
	If _lRet  // se confirma gravação                                                                  
		...
		If ZZE->(!DbSeek(xFilial("ZZE")+M->ZZE_NUMSOL))  // se não existir registro de solicitação      
			...
			ZZE->ZZE_TIPO   := "1"             // tipo da solicitação: 1-Novo Perfil
		...
		For n1 := 1 To Len(aCols)  // linhas  	          
			...
			If ZZF->(!DbSeek(xFilial("ZZF")+M->ZZE_NUMSOL+cSeqOa ))  // busca a linha da getdados
				...
				ZZF->ZZF_TIPO   := "1"             // tipo da solicitação: 1-Novo Perfil				
				MsUnlock()	        
			EndIf   
		Next 
		...
		If ZZF->(DbSeek(xFilial("ZZF")+M->ZZE_NUMSOL))  // posiciona registro
			Do While ZZF->(!Eof()) .And. ZZF->ZZF_NUM == M->ZZE_NUMSOL  // percorre o arquivo no intervalo
				...
				gcNivel	   := ZZF->ZZF_NIVEL                                // nível de acesso do usuário
...
User Function gVSolic()  // validação digitação da solicitação
...
cSubject := "Abertura da solicitação de inclusão de perfil de acesso para o NOVO usuário: "+M->ZZE_NMUSU  // Assunto da mensagem
cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como anexo
cBody    := U_gEWF001(cSubject,.T.,"","","",.F.,.F.,.F.,"","1",ZZE->ZZE_TIPO)  // chamada a função de montagem da mensagem para workflow
...
Static Function gWFGestor(gcGestor,gcModPerf)  // função de montagem do workflow para aprovação dos gestores
	...
	cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como     
