...
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³ gEspI011 ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Funções   ³ gEspI011 ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±³          ³ gWFHelpA ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±³          ³ gWFHelpE ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±³          ³ gWFCtrlC ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±³          ³ gWFGestor³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±³          ³ gWFUser  ³ Autor ³ George AC. Gonçalves ³ Data ³ 05/06/09  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descrição ³ Manutenção no Desbloqueio de Acesso do Usuário             ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ Específio: Projeto de concessão de acesso                  ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±³Partida   ³ Seleção da opção desbloqueio de usuário - Rotina gEspM013  ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
User Function gEspI011()  // Manutenção no Desbloqueio de Acesso de Usuário
...
If Len(aCols) > 0
	...
	If _lRet  // se confirma gravação                                                                  
		...
		If ZZE->(!DbSeek(xFilial("ZZE")+M->ZZE_NUMSOL))  // se não existir registro de solicitação      
			...
			ZZE->ZZE_TIPO   := "4"             // tipo da solicitação: 4-Desbloqueio Perfil			
		...
		For n1 := 1 To Len(aCols)  // linhas  	          
			...
			If ZZF->(!DbSeek(xFilial("ZZF")+M->ZZE_NUMSOL+cSeqOa ))  // busca a linha da getdados
				...
				ZZF->ZZF_TIPO   := "4"             // tipo da solicitação: 3-Desbloqueio Perfil							
				MsUnlock()	        
		...
		U_gEspP002(ZZE->ZZE_NUMSOL,"D")  // chamada a função que processa atualização de usuário/perfil							
...
cSubject := "Abertura da solicitação de desbloqueio de perfil de acesso para o usuário "+AllTrim(ZZE->ZZE_NMUSU)  // Assunto da mensagem
cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como anexo
cBody    := U_gEWF003(cSubject,.T.,.F.,"D")  // chamada a função de montagem da mensagem para workflow
...
cSubject := "Comunicação de desbloqueio de perfil de acesso para o usuário "+AllTrim(ZZE->ZZE_NMUSU)  // Assunto da mensagem
cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como anexo
cBody    := U_gEWF003(cSubject,.T.,.F.,"D")  // chamada a função de montagem da mensagem para workflow
...
cTo      := GetMV('MV_EWFMAX')  // Destinatário do envio do workflow para o helpdesk
cSubject := "Encerramento da solicitação de desbloqueio de perfil de acesso para o usuário "+AllTrim(ZZE->ZZE_NMUSU)  // Assunto da mensagem
cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como anexo
cBody    := U_gEWF003(cSubject,.T.,.F.,"D")  // chamada a função de montagem da mensagem para workflow
...
cSubject := "Seu acesso foi desbloqueado "+AllTrim(ZZE->ZZE_NMUSU)  // Assunto da mensagem
cAnexo   := ' '   // Caminho e nome do arquivo a ser enviado como anexo
cBody    := U_gEWF003(cSubject,.T.,.F.,"D")  // chamada a função de montagem da mensagem para workflow
