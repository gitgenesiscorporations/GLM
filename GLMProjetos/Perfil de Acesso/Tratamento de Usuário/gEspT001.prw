...
±±³Funções   ³ gEspT001 ³ Autor ³ George AC. Gonçalves ³ Data ³ 30/01/09  ³±±
...
±±³          ³ Arq1Acess³ Autor ³ George AC. Gonçalves ³ Data ³ 26/06/09  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
If Empty(ZZE->ZZE_CDUSU)  // se for inclusão de novo usuário
...
u2_nTipoImpr      := 2                                                                                                    // Tipo de impressão
u2_nFormato       := 1                                                                                                    // Formato
u2_nAmbiente      := 1                                                                                                    // Ambiente
///u2_lPrioridade    := If(Len(u1_aGrupo)>0,.T.,.F.)                                                                         // Prioridade para configuração do grupo/perfil
...
u3_aMenus         := gArrayXNU(u1_cUsrId)  // Chamada a função para para montar vetor com os menus de acesso
...
Else  // se não encontrar o ID do uauário
	...
	ZZE->ZZE_CDUSU := u1_cUsrId   // ID do usuário
	ZZE->ZZE_IDUSU := u1_cCdUser  // Código do usuário	
	MsUnlock()	        				
	...
	If SRA->(DbSeek(xFilial("SRA")+ZZE->ZZE_MATUSU))  // posiciona registro
		...
		SRA->RA_IDUSU := u1_cCdUser   // ID do usuário
		MsUnlock()	        				
    EndIf
...
Else  // se retorna o código do usuário
	...
	Else                     
		If ZZE->ZZE_UHOMO == "S"  // se usuário homônimo
			u1_cUsrId := StrZero(Val(aUsers[Len(aUsers)][1][1])+1,6)  // ID do usuário
		EndIf
...
Static Function gArrayXNU(g_cID) // função para para montar vetor com os menus de acesso
//g_cID = ID do usuário

Declare u3_aMenus[Len(aUsers[1][3])]

PSWORDER(1)  // muda ordem de índice
If PswSeek(g_cID) == .F.  // se não encontrar usuário no arquivo
	g_cID := "000000"
EndIf
	...
	If ZZF->(DbSeek(xFilial("ZZF")+ZZE->ZZE_NUMSOL+gCdMod))  // posiciona registro
		Do While ZZF->(!Eof()) .And. ZZF->ZZF_NUM == ZZE->ZZE_NUMSOL .And. ZZF->ZZF_CDMOD == gCdMod  // percorre o arquivo no intervalo
			If ZZF->ZZF_STATC == "1"  // aprovada
				...
				Else                 
					If g_cID <> "000000"  // se não for o usuário administrador
...
For gLn := 1 To Len(u1_aGrupo)
	...
	If ZZJ->(DbSeek(xFilial("ZZJ")+gcCdMod))  // posiciona ponteiro
		If ZZJ->ZZJ_MODCRI == "S"  // se módulo critico
			...
			ZZK->(DbSetOrder(1))  // muda ordem do índice
